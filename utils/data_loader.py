import pandas as pd
import streamlit as st
from pathlib import Path

# ===== Global Arrow-safe Fix =====
def make_arrow_safe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ensure all DataFrame columns are Arrow-compatible:
    - Converts mixed object columns to numeric if possible (int or float).
    - If numbers contain decimals, force float64.
    - Keeps text columns as strings.
    """
    for col in df.columns:
        if df[col].dtype == "object":
            numeric_df = pd.to_numeric(df[col], errors="coerce")
            if numeric_df.notna().sum() > 0:  # Found some numeric values
                if (numeric_df % 1 != 0).any():
                    df[col] = numeric_df.astype("float64")
                else:
                    df[col] = numeric_df.astype("int64")
            else:
                df[col] = df[col].astype(str)
    return df

@st.cache_data
def load_all_data():
    """
    Load all data for the GOAT analysis.

    Priority:
      1) Try to read CSVs from ./data
      2) If any required frame is missing/unreadable, fall back to the in-code DataFrames
    """
    DATA_DIR = Path("data")

    def find_csv(*needles):
        if not DATA_DIR.exists():
            return None
        for p in DATA_DIR.glob("*.csv"):
            name = p.name.lower()
            if any(n.lower() in name for n in needles):
                return p
        return None

    try:
        csv_candidates = {
            "achievements":   find_csv("honours", "trophies", "awards", "achiev"),
            "career_stats":   find_csv("career_stats", "career_summary", "career"),
            "club_goals":     find_csv("club_goals", "club_performance", "ucl_splits"),
            "international":  find_csv("international_summary", "international", "intl"),
            "fame":           find_csv("fame", "google_trends", "followers"),
            "league_stats":   find_csv("league_stats", "season_by_season_club", "league"),
            "physical":       find_csv("physical_attributes", "physical"),
            "tactical":       find_csv("tactical_attributes", "tactical_attributes"),
            "clutch":         find_csv("clutch_performance", "clutch_contributions", "clutch"),
            "season_goals":   find_csv("goals_by_season", "season_goals", "season_by_season"),
        }

        loaded = {}
        read_ok = True
        for key, path in csv_candidates.items():
            if path is None:
                read_ok = False
                break
            df = pd.read_csv(path)
            loaded[key] = make_arrow_safe(df)

        if read_ok and len(loaded) == len(csv_candidates):
            st.sidebar.success(f"✅ Data loaded from CSV files in ./data ({len(loaded)} datasets)")
            return loaded

        st.sidebar.info("ℹ️ Some CSVs not found or incomplete — using embedded fallback values.")

    except Exception as e:
        st.sidebar.warning(f"⚠️ CSV load issue: {e} — using embedded fallback values.")

    # === FALLBACK: In-code data ===
    achievements = pd.DataFrame({
        'category': ['Ballon d\'Or', 'Golden Boot', 'Champions League Top Scorer', 'FIFA Best', 'UEFA Player of Year', 'Liga MVP', 'World Cup Golden Ball'],
        'messi': [8, 6, 6, 3, 3, 6, 1],
        'ronaldo': [5, 4, 7, 2, 3, 2, 0]
    })

    career_stats = pd.DataFrame({
        'player': ['Messi', 'Ronaldo'],
        'goals': [815, 895],
        'assists': [377, 236],
        'matches': [1069, 1205],
        'trophies': [44, 35],
        'ballonDors': [8, 5],
        'goalsPer90': [0.76, 0.74],
        'assistsPer90': [0.35, 0.20],
        'conversionRate': [17.8, 15.2],
        'shotAccuracy': [45.2, 42.8],
        'penaltyConversion': [77.8, 84.3],
        'dribblesCompleted': [2847, 1456],
        'keyPasses': [4521, 2847],
        'chancesCreated': [3254, 2108],
        'teamTrophies': [44, 35],
        'individualAwards': [47, 34]
    })

    club_goals = pd.DataFrame({
        'club': ['Barcelona', 'PSG', 'Inter Miami', 'Man United', 'Real Madrid', 'Juventus', 'Al Nassr', 'Sporting CP'],
        'messi': [672, 32, 25, 0, 0, 0, 0, 0],
        'ronaldo': [0, 0, 0, 145, 451, 101, 68, 5]
    })

    international = pd.DataFrame({
        'player': ['Messi', 'Ronaldo'],
        'country': ['Argentina', 'Portugal'],
        'matches': [183, 206],
        'goals': [108, 130],
        'assists': [56, 43],
        'trophies': [2, 2],
        'minutesPerGoal': [132, 130]
    })

    fame = pd.DataFrame({
        'player': ['Messi', 'Ronaldo'],
        'instagramFollowers': [500000000, 615000000],
        'facebookFollowers': [115000000, 170000000],
        'twitterFollowers': [46000000, 110000000],
        'youtubeSubscribers': [2500000, 55000000],
        'googleSearchVolume': [45000000, 67000000],
        'sponsorshipIncomeUSD': [55000000, 70000000],
        'merchSalesUSD': [125000000, 185000000],
        'globalFanBaseM': [450, 520],
        'influenceRank': [2.1, 1.8]
    })

    league_stats = pd.DataFrame({
        'league': ['La Liga', 'Premier League', 'Serie A', 'Ligue 1', 'MLS', 'Saudi Pro League'],
        'messiGoals': [474, 0, 0, 16, 11, 0],
        'ronaldoGoals': [311, 103, 81, 0, 0, 35],
        'messiApps': [520, 0, 0, 67, 14, 0],
        'ronaldoApps': [292, 236, 98, 0, 0, 42]
    })

    physical = pd.DataFrame({
        'player': ['Messi', 'Ronaldo'],
        'topSpeed_kmh': [32.5, 34.6],
        'strength_score': [75, 95],
        'stamina_score': [88, 92],
        'agility_score': [99, 85]
    })

    tactical = pd.DataFrame({
        'attribute': ['Vision', 'Passing', 'Dribbling', 'Finishing', 'Positioning', 'Aerial Ability'],
        'messi': [98.5, 97.8, 99.2, 95.4, 94.6, 72.3],
        'ronaldo': [88.7, 85.2, 89.4, 97.8, 96.5, 98.1]
    })

    clutch = pd.DataFrame({
        'category': ['Final Goals', 'Penalty Shootout Goals', 'Last Minute Goals', 'Hat-tricks in Finals', 'Big Game Goals'],
        'messi': [26, 4, 23, 2, 89],
        'ronaldo': [22, 7, 31, 3, 95]
    })

    years = list(range(2005, 2024))
    messi_goals_by_year = [1, 8, 17, 16, 38, 47, 53, 73, 60, 41, 58, 54, 37, 51, 36, 31, 38, 11, 21]
    ronaldo_goals_by_year = [9, 12, 23, 42, 33, 40, 60, 55, 51, 61, 48, 51, 42, 44, 37, 36, 24, 18, 35]

    season_goals = pd.DataFrame({
        'year': years,
        'messiGoals': messi_goals_by_year,
        'ronaldoGoals': ronaldo_goals_by_year,
        'messiAssists': [a//2 for a in messi_goals_by_year],
        'ronaldoAssists': [a//3 for a in ronaldo_goals_by_year]
    })

    data_dict = {
        'achievements': achievements,
        'career_stats': career_stats,
        'club_goals': club_goals,
        'international': international,
        'fame': fame,
        'league_stats': league_stats,
        'physical': physical,
        'tactical': tactical,
        'clutch': clutch,
        'season_goals': season_goals
    }

    for k, df in data_dict.items():
        data_dict[k] = make_arrow_safe(df)

    st.sidebar.success(f"✅ Data loaded from embedded values (fallback). Datasets: {len(data_dict)}")
    return data_dict
